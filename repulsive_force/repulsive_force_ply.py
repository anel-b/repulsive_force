...
class RepulsiveForcePublisher(Node):
    ...
    def get_point_cloud(self):
        ...
        # Extract XYZ and RGB values from point cloud data
        points_data = point_cloud_data[:, :3]
        colors_data = point_cloud_data[:, 3:]

        # Remove points with NaN XYZ values and replace NaN RGB values with black
        mask = ~np.isnan(points_data).any(axis=1)
        points_data = points_data[mask]
        colors_data = colors_data[mask]
        mask = np.isnan(colors_data).any(axis=1)
        colors_data[mask] = np.array([0.0, 0.0, 0.0])
