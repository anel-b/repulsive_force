...
class RepulsiveForcePublisher(Node):
    ...
    def get_nearest_points(self, point_cloud: o3d.geometry.PointCloud):
        ...
        [k, idx, dist] = point_cloud_tree.search_knn_vector_3d(obstacle, 20)
        if k != 0:
            k_obstacles = (np.asarray(point_cloud.points))[idx]
            k_distances = np.sqrt(dist)
        else:
        ...
        distances = np.append(distance, k_distances)
    ...
    def compute_repulsive_force(self, obstacles, distances):
        ...
        safe_distance = 0.2
        danger_distance = 0.05
        magnitude_limit = 50.0
        gain = 10.0

        # Compute repulsive force for k nearest points with limited magnitude
        k = len(obstacles)
        F_repulsion = np.zeros((k, 3))
        indices = np.ones(k, dtype=bool)
        for i in range(k):
            ...
            if distances[i] < safe_distance:
                if distances[i] < danger_distance:
                    F_repulsion[i] = magnitude_limit * unit_vector
                else:
                    magnitude = gain * np.log((safe_distance - danger_distance) / (distances[i] - danger_distance))
                    F_repulsion[i] = min(magnitude, magnitude_limit) * unit_vector
            else:
                F_repulsion[i] = np.zeros(3)
                indices[i] = False

        # Remove repulsive forces with zero magnitude
        F_repulsion = F_repulsion[indices]
        ...
        # Find direction of minimum variance and its not aligned with maximum direction of repulsive force
        ...
        # Compute tangential component aligned most with repulsive force
        v_pos = min_eigenvalue * min_eigenvector
        v_neg = -1 * min_eigenvalue * min_eigenvector
        v_pos_proj = np.dot(F_repulsion, v_pos)
        v_neg_proj = np.dot(F_repulsion, v_neg)
        if abs(v_pos_proj) >= abs(v_neg_proj):
            F_tangential_repulsion = v_pos
        else:
            F_tangential_repulsion = v_neg

        # Compute repulsive force with tangential component
        F_repulsion = F_repulsion + 0.5 * np.linalg.norm(F_repulsion) * F_tangential_repulsion
