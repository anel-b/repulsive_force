...
CallbackReturn CartesianImpedanceController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/) {
  ...
  try {
    ...
    //START repulsive_force
    repulsive_force_subscriber = get_node()->create_subscription<geometry_msgs::msg::WrenchStamped>(
      "repulsive_force_topic", qos_profile,
      std::bind(&CartesianImpedanceController::repulsive_force_callback, this, std::placeholders::_1));
    //END repulsive_force
  }
...
//START repulsive_force
void CartesianImpedanceController::repulsive_force_callback(const std::shared_ptr<geometry_msgs::msg::WrenchStamped> msg) {
  F_repulsion[0] = msg->wrench.force.x;
  F_repulsion[1] = msg->wrench.force.y;
  F_repulsion[2] = msg->wrench.force.z;
  F_repulsion[3] = msg->wrench.torque.x;
  F_repulsion[4] = msg->wrench.torque.y;
  F_repulsion[5] = msg->wrench.torque.z;
}
//END repulsive_force
...
//START repulsive_force
void CartesianImpedanceController::change_target_position() {
  position_d_target_(1) *= -1;
}
//END repulsive_force
...
controller_interface::return_type CartesianImpedanceController::update(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {  
  ...
  //START repulsive_force
  if (F_repulsion.norm() > 0 && error.norm() > 0.1) {
    F_impedance *= 0.5;
  }
  //END repulsive_force
  ...
  if (outcounter % 1000/update_frequency == 0){
    ...
    */
    //START repulsive_force
    Eigen::Matrix<double, 6, 1> F_total = F_impedance + F_repulsion;
    std::cout << "--------" << std::endl;
    ...
    std::cout << "F_total = [" << F_total[0] << " " << F_total[1] << " " << F_total[2] << "]" << std::endl;
    //END repulsive_force
  }
  //START repulsive_force
  if (outcounter % 5000 == 0) {
